...
def makePatches():
    ...
    for i in range(len(srcPath)):
        for x in range(0 + args.offset, (size - args.pat_size_b), args.stride):
            for y in range(0 + args.offset, (size - args.pat_size_a), args.stride):
                count += 1
    ...
    np.save(os.path.join(args.save_dir, "raw_float_sino_pats"), inputs)
